...
class UserServices {
  ...
  register = async (req: Request, res: Response, next: NextFunction): Promise<Response | void> => {
    try {
      ...
      return successHandler({ res, status: 201, message: "User registered successfully" });
    } catch (error) { next(error); }
  };

  confirmEmail = async (req: Request, res: Response, next: NextFunction): Promise<Response | void> => {
    try {
  ...
  resendOtp = async (req: Request, res: Response, next: NextFunction): Promise<Response | void> => {
    try {
      ...
      const otp = generateOtp({ email: user.email, subject: user.confirmedAt ? SubjectEnum.resetPassword : SubjectEnum.registration });
      if (!user.confirmedAt) {
  ...
  login = async (req: Request, res: Response, next: NextFunction): Promise<Response | void> => {
    try {
      ...
      if (!user) throw new NotFoundError("User not found");
      if (!user.confirmedAt) throw new ConflictError("Please confirm your email first");
      if (user.password && !(await compareHash(password, user.password))) throw new BadRequestError("Invalid credentials");
  ...
  loginWithGoogle = async (req: Request, res: Response, next: NextFunction): Promise<Response | void> => {
    try {
      const { token } = req.body;
      const { name, email } = await verifyGoogleToken(token);

      let user: any = await this.userModel.findOne({ filter: { email } as any, options: { lean: false } });

      if (!user) {
        const created = await this.userModel.create({
          data: [{ username: name, email, provider: ProvidersEnum.google as any, confirmedAt: new Date(), isLogged: true, role: RoleEnum.user as any }]
        });
  ...
  discordLogin = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      let user: any = await this.userModel.findOne({ filter: { discordId: discordUser.id } as any, options: { lean: false } });

      if (!user) {
        const created = await this.userModel.create({
          data: [{ username: discordUser.username, discordId: discordUser.id, provider: ProvidersEnum.discord as any, confirmedAt: new Date(), isLogged: true, role: RoleEnum.user as any }]
        });
        user = created ? created[0] : null;
      } else {
        user.isLogged = true;
  ...
  resetPassword = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const user: any = await this.userModel.findOne({ filter: { email } as any, options: { lean: false } });
      if (!user?.confirmForgetPasswordAt) throw new ConflictError("Please verify your email first");
  ...
  logout = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      await this.userModel.updateOne({ filter: { _id: req.user?._id } as any, update });
